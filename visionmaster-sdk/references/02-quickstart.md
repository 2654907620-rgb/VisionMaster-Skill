<!-- source: chm_core.md (编程引导/事件/示例程序介绍) -->
# VM .NET SDK 编程引导

VM算法平台SDK基于方案、流程和模块工具等进行对象级封装，可通过方案、流程和模块工具中各个对象的功能接口进行相应的数据交互与运行控制。同时提供相应的流程配置控件、参数配置控件、渲染控件、全局工具控件以及前端运行界面控件等，方便进行方案编辑、参数配置以及渲染展示。SDK提供C++和C#两套接口，本文档介绍如何通过C#接口调用VisionMaster算法平台SDK的外部接口。

## 速览

- 支持 .NET Framework 4.6.1 及以上，不支持 .NET Core。
- 核心命名空间：`VM.Core`（方案/流程/模块对象）、`VM.PlatformSDKCS`（底层实现与数据类型）。
- 单例入口：`VmSolution.Instance`；加载方案用 `VmSolution.Load("绝对路径", "密码")`。
- 通过索引器按全名获取对象：`VmSolution.Instance["流程1.图像源1"]`，需强制类型转换。
- 方案/流程/Group/模块均提供同步执行（`SyncRun` / `Run`）与连续执行（`ContinuousRunEnable`）能力。
- 设置输入类接口（图像、ROI、掩膜）`仅当次执行有效`，执行完成后清空，再次执行需再次设置。
- 控件分 WinForm（`VMControls.Winform.Release`）与 WPF（`VMControls.WPF.Release`）两套库。
- 引用动态库后需将"复制本地"设为 False；Any CPU 平台需去掉"首选 32 位"。
- 退出程序前调用 `VmSolution.Instance?.Dispose()` 释放资源，避免在析构函数中调用。
- 事件通过 `+=` 订阅（如 `VmSolution.OnWorkStatusEvent`），回调中避免耗时操作。
- 中英文切换修改 `..\Development\V4.x\ComControls\Assembly\LangCFG\LanguageSet.cfg`（`zh-cn` / `en-us`）。
- 官方提供平台型（方案/流程/Group/圆查找/运行界面/获取结果）与应用型（OCR/定位/深度学习）示例。
...
- VS运行环境：VS2015 及以上版本。
- 平台目标选择【Any CPU】时，需去除勾选【首选 32 位】。
- 手动添加 VM 相关动态库引用后，需设置库属性中的【复制本地】为 **False**。
...
- `VMControls.RenderInterface.dll`：带渲染功能控件的基础库，如 ROI、图形显示等。
- `VMControls.Winform.Release.dll`：Winform 控件的基础库，包含 Winform 控件的对外接口。
- `VMControls.WPF.Release.dll`：包含 WPF 控件的对外接口。

**开发方式说明（不包含具体模块，如使用到模块，在以下基础上导入使用到的对应模块 dll 即可）：**

- 纯调接口方式（无控件）需导入以下 **2 个 dll**：
  - `VM.Core.dll`
  - `VM.PlatformSDKCS.dll`
- Winform 控件需导入以下 **6 个 dll**：
  - `VM.Core.dll`
  - `VM.PlatformSDKCS.dll`
...
- 推荐自动工具方式添加动态库引用，工具路径：`..\Development\V4.x\ComControls\Tool\ImportRef.exe`。
- 也可手动通过右键菜单【选择项】打开【选择工具箱项】界面，手动浏览打开对应版本的 `VMControls.WPF.Release.dll`（WPF）或 `VMControls.Winform.Release.dll`（WinForm），勾选组件以添加对应控件。
- 正常安装后，默认可从 WPF / WinForm 工具箱中拖出控件。

### 控件工具箱添加方法

- WPF：若工具箱中无对应控件，可通过右键菜单【选择项】打开【选择工具箱项】界面，手动浏览打开对应版本 `VMControls.WPF.Release.dll` 动态库文件，最后勾选 WPF 组件以添加对应控件。
- WinForm：若工具箱中无对应控件，可通过右键菜单【选择项】打开【选择工具箱项】界面，手动浏览打开对应版本 `VMControls.Winform.Release.dll` 动态库文件，最后勾选 .NET Framework 组件以添加对应控件。
- 正常安装后，默认可从相应工具箱中拖出控件；也可通过 xaml 设计器方式直接集成 WPF 控件。

### 控件中英文切换

- 修改 `..\Development\V4.x\ComControls\Assembly\LangCFG` 目录下的配置文件 `LanguageSet.cfg`。
- `zh-cn` 表示中文，`en-us` 表示英文。
- 注意：VM4.0/4.1 的配置文件在 debug 路径下的 LangCFG 文件夹中；VM4.2/4.3 因 debug 中未进行拷贝操作，需修改 `..\Development\V4.x\ComControls\Assembly\LangCFG\LanguageSet.cfg`。

### 注意事项

- 程序退出前，可调用接口释放 VM 相关资源，请避免在析构函数中调用接口。
...
VM SDK 以对象树的方式组织：方案（Solution）包含流程（Procedure），流程包含模块（Module）；流程内可嵌套 Group（组合模块）；全局模块（全局变量/通信管理/数据队列/全局相机/全局光源等）与方案平级挂载。

**核心获取方式：**

- `VmSolution.Instance`：方案单例对象入口。
- 索引器按全名获取对象：`VmSolution.Instance["流程1"]` 获取流程；`VmSolution.Instance["流程1.图像源1"]` 获取模块；`VmSolution.Instance["流程1.组合模块1"]` 获取 Group；`VmSolution.Instance["全局变量1"]` 获取全局模块。获取后需强制转换为具体类型（如 `VmProcedure`、`ImageSourceModuleTool`、`IMVSGroupTool`、`GlobalVariableModuleTool`）。
- `VmSolution.Instance.GetAllProcedureObjects(ref List<VmProcedure>)`：获取方案所有流程对象列表。
- `VmSolution.Instance.GetAllProcedureList()`：获取方案所有流程信息（`ProcessInfoList`）。
- `VmSolution.Instance.GetAllModuleList()`：获取方案所有模块信息（`ModuleInfoList`）。
- 流程级：`VmProcedure.GetAllModuleList()`（含 Group 内模块）、`VmProcedure.GetProcedureModuleList()`（本层级，不含 Group 内部模块）。
- Group：`IMVSGroupTool` 通过 `VmSolution.Instance["..."]` 获取，或使用 `IMVSGroupTool.LoadIndependentGroup("绝对路径")` 加载独立 Group（与方案并列，非所属关系，无法用方案对象获取或操作）。
- 全局模块：作为方案内的特殊模块对象，通过 `VmSolution.Instance["全局变量1"]` 等方式获取（中文 VM 用中文名，英文 VM 用英文名称如 `"Global Variable1"`）。
...
Group 相关操作主要包括导入 Group、获取模块列表、禁用/启用 Group、自执行、导出 Group、删除 Group 等。

### 接口调用流程

Group 相关操作接口调用流程如下图所示。其中，蓝色框内容表示必选操作，橙色框内容表示可选操作。

### 示例代码

Group 相关操作示例代码如下，仅供参考。
...
VM SDK 通过事件回调反馈方案/流程/Group/模块的运行状态与结果数据。事件通过 `+=` 订阅，回调中避免进行同步渲染等耗时操作，以免阻塞下次运行。
...
VM 提供平台型示例程序（展示各控件与对象接口用法）与应用型示例程序（展示实际视觉应用二次开发）。

### 平台型示例程序

* **方案（SolutionControl Demo）**：主要展示方案配合流程配置控件如何使用，以及方案对象中部分接口如何调用。功能区域：1 方案操作接口（加载、保存、关闭）、2 消息显示区、3 流程配置区/主界面区。
* **流程（ProcessControl Demo）**：主要展示方案中流程配合流程配置控件如何使用，同时还可导入/导出流程，控制流程执行、禁用和启用。功能区域：1 方案操作接口、2 流程操作接口、3 流程控制接口、4 消息显示区、5 流程配置区。
* **Group（GroupControl Demo）**：主要展示独立的 Group 如何导入/导出，绑定参数配置控件如何修改参数，绑定渲染控件如何展示渲染效果。功能区域：1 Group 操作、2 Group 显示区、3 消息显示区、4 结果渲染区。
* **圆查找（CircleFind Demo）**：主要展示如何加载包含圆查找模块的方案，绑定参数配置控件如何修改配置参数，以及如何展示渲染效果。功能区域：1 方案操作接口、2 模块操作区、3 消息显示区、4 参数配置区、5 结果渲染区。
* **运行界面（FrontendControl Demo）**：主要展示方案中运行界面相关操作如何进行二次开发。功能区域：1 方案操作接口、2 运行界面控制区、3 消息显示区、4 运行界面渲染区。
* **获取结果（GetResultControl Demo）**：主要展示如何通过二次开发在回调中获取数据结果。功能区域：1 方案操作接口、2 获取结果操作接口、3 渲染、4 消息显示区、5 流程配置区、6 图像渲染区。

### 应用型示例程序

* **OCRDemoCs（字符识别示例）**：用于展示字符识别相关应用方面的二次开发。示例方案路径：`./PlatformSDKSampleCS/OCRDemoCs/OCRDemo.sol`。涉及控件：VmRenderControl、VmMainViewConfigControl、VmGlobalToolControl。
* **LocateDemoCs（抓取定位示例）**：用于展示抓取定位相关应用方面的二次开发。示例方案路径：`./PlatformSDKSampleCS/LocateDemoCs/LocateDemoCs.sol`。涉及控件：VmRenderControl、VmMainViewConfigControl、VmGlobalToolControl。
* **DeepLearningDemoCs（深度学习示例）**：用于展示深度学习相关应用方面的二次开发。示例方案路径：`./PlatformSDKSampleCS/DeepLearningDemoCs/DeepLearningDemo.sol`。涉及控件：VmRenderControl、VmMainViewConfigControl、VmGlobalToolControl。

应用型示例程序均通过流程输出参数 `out`（在流程"输出设置"中订阅、格式化模块汇总）解耦获取结果；通过渲染控件绑定 `ModuleSource` 显示渲染；通过 `VmSolution.OnWorkStatusEvent` 或 `VmProcedure.OnWorkEndStatusCallBack` 注册回调获取结果。
