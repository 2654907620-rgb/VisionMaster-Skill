<!-- src:class_i_m_v_s_peak_find_modu_cs_1_1_peak_find_result.html -->
<!-- path:接口函数 > 定位 > 顶点检测 > PeakFindResult -->
# PeakFindResult类 参考 定位 » 顶点检测

顶点检测结果
更多...

继承自 CModuleResultBase .

|  |  |
| --- | --- |
| 属性 | |
| int | ModuStatus `[get]` |
|  | 模块状态 更多... |
|  | |
| float | PeakScore `[get]` |
|  | 顶点分数 更多... |
|  | |
| int | PeakPolarity `[get]` |
|  | 顶点极性 更多... |
|  | |
| float | PeakBias `[get]` |
|  | 顶点距离 更多... |
|  | |
| PointF | TopPoint `[get]` |
|  | 顶点 更多... |
|  | |
| List< int > | EdgeStatus `[get]` |
|  | 边缘状态 更多... |
|  | |
| int | CaliperEdgeNum `[get]` |
|  | 边缘个数 更多... |
|  | |
| List< float > | CaliperEdgeScore `[get]` |
|  | 分数 更多... |
|  | |
| List< int > | CaliperEdgePolarity `[get]` |
|  | 边缘极性 更多... |
|  | |
| List< float > | CaliperEdgeBias `[get]` |
|  | 边缘位置 更多... |
|  | |
| List< PointF > | EdgePoint `[get]` |
|  | 边缘点 更多... |
|  | |
| ImageBaseData | OutputMask `[get]` |
|  | 输出掩膜 更多... |
|  | |
| RectBox | ROI `[get]` |
|  | 检测区域 更多... |
|  | |
| List< int > | BlindPolygonPointNum `[get]` |
|  | 多边形点数 更多... |
|  | |
| List< PointF > | BlindPolygonPoints `[get]` |
|  | 多边形点集 更多... |
|  | |
| List< string > | BlindPolygonString `[get]` |
|  | 屏蔽区字符串 更多... |
|  | |
| 属性 继承自 CModuleResultBase | |
| uint | ErrorCode `[get]` |
|  | 错误码 更多... |
|  | |
| float | ModuRunTime `[get]` |
|  | 模块执行耗时 更多... |
|  | |

## 详细描述

顶点检测结果

## 属性说明

## ◆ ModuStatus

|  |  |  |
| --- | --- | --- |
| |  | | --- | | int ModuStatus | | get |

模块状态

## ◆ PeakScore

|  |  |  |
| --- | --- | --- |
| |  | | --- | | float PeakScore | | get |

顶点分数

## ◆ PeakPolarity

|  |  |  |
| --- | --- | --- |
| |  | | --- | | int PeakPolarity | | get |

顶点极性

## ◆ PeakBias

|  |  |  |
| --- | --- | --- |
| |  | | --- | | float PeakBias | | get |

顶点距离

## ◆ TopPoint

|  |  |  |
| --- | --- | --- |
| |  | | --- | | PointF TopPoint | | get |

顶点

## ◆ EdgeStatus

|  |  |  |
| --- | --- | --- |
| |  | | --- | | List<int> EdgeStatus | | get |

边缘状态

## ◆ CaliperEdgeNum

|  |  |  |
| --- | --- | --- |
| |  | | --- | | int CaliperEdgeNum | | get |

边缘个数

## ◆ CaliperEdgeScore

|  |  |  |
| --- | --- | --- |
| |  | | --- | | List<float> CaliperEdgeScore | | get |

分数

## ◆ CaliperEdgePolarity

|  |  |  |
| --- | --- | --- |
| |  | | --- | | List<int> CaliperEdgePolarity | | get |

边缘极性

## ◆ CaliperEdgeBias

|  |  |  |
| --- | --- | --- |
| |  | | --- | | List<float> CaliperEdgeBias | | get |

边缘位置

## ◆ EdgePoint

|  |  |  |
| --- | --- | --- |
| |  | | --- | | List<PointF> EdgePoint | | get |

边缘点

## ◆ OutputMask

|  |  |  |
| --- | --- | --- |
| |  | | --- | | ImageBaseData OutputMask | | get |

输出掩膜

## ◆ ROI

|  |  |  |
| --- | --- | --- |
| |  | | --- | | RectBox ROI | | get |

检测区域

## ◆ BlindPolygonPointNum

|  |  |  |
| --- | --- | --- |
| |  | | --- | | List<int> BlindPolygonPointNum | | get |

多边形点数

## ◆ BlindPolygonPoints

|  |  |  |
| --- | --- | --- |
| |  | | --- | | List<PointF> BlindPolygonPoints | | get |

多边形点集

## ◆ BlindPolygonString

|  |  |  |
| --- | --- | --- |
| |  | | --- | | List<string> BlindPolygonString | | get |

屏蔽区字符串
